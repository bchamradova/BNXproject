...
class LineEquation:
    ...
    '''def getPointsFromStartToEnd(self):
        points = []
        for distance in range(1,self.getDistanceBetweenTwoPoints(self.point1, self.point2)):
            points.append(self.getCoordinatesInDistanceFromFirstPoint(distance))
        return points'''

    def getPointsFromStartToEnd(self):
        points = []
        for i in range(self.point2[1] - self.point1[1] + 1):
            try:
                points.append((self.getXCoordinateFromY(self.point1[1] + i), self.point1[1] + i))
            except SlopeException:
                points.append((self.point1[0], self.point1[1] + i))
        return points
